...
Sometimes we have spatial features (like points or polygons) that we want to aggregate or index using geohashes. The `gh_covering` function produces a grid of geohashes that overlap with the spatial object. Thanks to native `sf` integration, `gh_covering` automatically handles coordinate transformations—you can supply an object in any coordinate reference system (such as projected UTM coordinates), and `gh_covering` will transform it internally to generate the grid and transform the resulting geohash cover back to match your original CRS.
