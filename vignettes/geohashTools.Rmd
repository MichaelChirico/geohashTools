...
```{r olinda, fig.width = 4, fig.height = 4, out.width = '\\textwidth'}
...
```{r olinda_covering_plot, fig.width = 4, fig.height = 4, out.width = '\\textwidth'}
ggplot() +
  geom_sf(data = olinda_4326) +
  geom_sf(data = olinda_gh, fill = NA, colour = 'red', linewidth = 0.5) +
  geom_sf_text(data = olinda_gh, aes(label = rownames(olinda_gh)), colour = 'blue', size = 4.0) +
  theme_void()
```

Alternatively, using `gh_covering` with the parameter `minimal = TRUE` will only create geohashes for intersecting objects.
```{r olinda_minimal, fig.width = 4, fig.height = 4, out.width = '\\textwidth'}
olinda_minimal_gh <- gh_covering(olinda_4326, precision = 5L, minimal = TRUE)

ggplot() +
  geom_sf(data = olinda_4326) +
  geom_sf(data = olinda_minimal_gh, fill = NA, colour = 'red', linewidth = 0.5) +
  geom_sf_text(data = olinda_minimal_gh, aes(label = rownames(olinda_minimal_gh)), colour = 'blue', size = 4.0) +
  theme_void()
```
