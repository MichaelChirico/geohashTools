...
Let's use the classic North Carolina SIDS dataset included in the `sf` package.
```{r nc_sids, fig.width = 6, fig.height = 3, out.width = '\\textwidth'}
...
nc_4326 <- st_transform(nc, crs = 4326L)

ggplot() +
  geom_sf(data = nc_4326, aes(fill = SID74)) +
  scale_fill_viridis_c() +
  theme_void()
```
By default, `gh_covering` creates a grid that covers the extent of the bounding box for the spatial object. Here we use `precision = 3L` (cell dimensions roughly 156km x 156km) to cover the state cleanly.
```{r nc_covering}
nc_gh <- gh_covering(nc_4326, precision = 3L)
nc_gh
```

We can visualize what this looks like with geohashes overlayed on top.

```{r nc_covering_plot, fig.width = 6, fig.height = 3, out.width = '\\textwidth'}
ggplot() +
  geom_sf(data = nc_4326) +
  geom_sf(data = nc_gh, fill = NA, colour = 'red', linewidth = 0.5) +
  geom_sf_text(data = nc_gh, aes(label = rownames(nc_gh)), colour = 'blue', size = 4.0) +
  theme_void()
```

Alternatively, using `gh_covering` with the parameter `minimal = TRUE` will only create geohashes for intersecting objects.
```{r nc_minimal, fig.width = 6, fig.height = 3, out.width = '\\textwidth'}
nc_minimal_gh <- gh_covering(nc_4326, precision = 3L, minimal = TRUE)

ggplot() +
  geom_sf(data = nc_4326) +
  geom_sf(data = nc_minimal_gh, fill = NA, colour = 'red', linewidth = 0.5) +
  geom_sf_text(data = nc_minimal_gh, aes(label = rownames(nc_minimal_gh)), colour = 'blue', size = 4.0) +
  theme_void()
```
