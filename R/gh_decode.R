...
#'
#' @param geohashes `character` or `factor` vector or of input geohashes. There's no need for all inputs to be of
#'   the same precision.
#' @param include_delta `logical`; should the cell half-width delta be included in the output?
#' @param coord_loc `character` specifying where in the cell points should be mapped to; cell centroid is mapped by
...
#'
#' \item{latitude}{ `numeric` vector of latitudes (y-coordinates) corresponding to the input `geohashes`, with
#'   within-cell position dictated by `coord_loc` }
#' \item{longitude}{ `numeric` vector of longitudes (x-coordinates) corresponding to the input `geohashes`, with
#'   within-cell position dictated by `coord_loc` }
#' \item{delta_latitude}{ `numeric` vector of cell half-widths in the y direction (only included if `include_delta`
#'   is `TRUE` }
#' \item{delta_longitude}{ `numeric` vector of cell half-widths in the x direction (only included if `include_delta`
#'   is `TRUE` }
#'
#' @references
